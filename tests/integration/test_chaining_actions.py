...
from tests.integration.conftest import impersonate_account, stop_impersonating, send_contract_tx
...
def test_chaining_action(mainnet_fork_web3, weiroll_vm, tuple_helper):
    ...
    try:
        ...
        yfi_address = Web3.to_checksum_address("0x0bc529c00C6401aEF6D220BE8C6Ea1667F6Ad93e")
        crv_yfi_weth_address = Web3.to_checksum_address("0x29059568bB40344487d62f7450E78b8E6C74e0e5")
        curve_swap_address = Web3.to_checksum_address("0xC26b89A667578ec7b3f11b2F98d6Fd15C07C54ba")
        ...
        crv_yfi_weth = mainnet_fork_web3.eth.contract(address=crv_yfi_weth_address, abi=erc20_abi)
        
        # Curve swap ABI
        curve_swap_abi = [
            {"name":"add_liquidity","outputs":[{"type":"uint256","name":""}],"inputs":[{"type":"uint256[2]","name":"amounts"},{"type":"uint256","name":"min_mint_amount"}],"stateMutability":"nonpayable","type":"function"}
        ]
        curve_swap = mainnet_fork_web3.eth.contract(address=curve_swap_address, abi=curve_swap_abi)
        ...
        assert response.ok and response.status_code == 200, f"1inch API error: {response.text}"
        ...
        one_inch_abi = [
            {"inputs":[{"internalType":"address","name":"caller","type":"address"}],"name":"swap","outputs":[{"internalType":"uint256","name":"returnAmount","type":"uint256"},{"internalType":"uint256","name":"spentAmount","type":"uint256"}],"stateMutability":"nonpayable","type":"function"}
        ]
        one_inch = mainnet_fork_web3.eth.contract(address=one_inch_address, abi=one_inch_abi)
