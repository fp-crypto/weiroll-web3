...
from tests.integration.conftest import convert_to_wei, impersonate_account, stop_impersonating, send_contract_tx
...
def test_swaps(weiroll_vm, mainnet_fork_web3, setup_accounts, whale_account):
    ...
    crvseth_address = Web3.to_checksum_address("0xc5424B857f758E906013F3555Dad202e4bdB4567")
    susd_address = Web3.to_checksum_address("0x57Ab1ec28D129707052df4dF418D58a2D46d5f51")
    yvweth_address = Web3.to_checksum_address("0xa258C4606Ca8206D8aA700cE2143D7db854D168c")
    sushi_router_address = Web3.to_checksum_address("0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F")
    univ3_router_address = Web3.to_checksum_address("0xE592427A0AEce92De3Edee1F18E0157C05861564")
    ...
    crvseth_contract = mainnet_fork_web3.eth.contract(address=crvseth_address, abi=crvseth_abi)
    susd_contract = mainnet_fork_web3.eth.contract(address=susd_address, abi=erc20_abi)
    yvweth_contract = mainnet_fork_web3.eth.contract(address=yvweth_address, abi=yvweth_abi)
    sushi_router = mainnet_fork_web3.eth.contract(address=sushi_router_address, abi=sushi_router_abi)
    univ3_router = mainnet_fork_web3.eth.contract(address=univ3_router_address, abi=univ3_router_abi)
    ...
    try:
        ...
        impersonate_account(mainnet_fork_web3, whale, fund_amount=convert_to_wei(1), dev_account=dev)
        ...
        if yvweth_balance < int(2e18) or weth_balance < int(1.118383e18):
            pytest.skip(f"Whale doesn't have enough tokens. yvWETH: {yvweth_balance}, WETH: {weth_balance}")
        ...
        planner.add(
            w_sushi_router.swapExactTokensForTokens(
                weth_bal, 0, [w_weth.address, w_susd.address], weiroll_vm.address, 2**256 - 1
            )
        )
        ...
        # Verify results
        final_weth_balance = weth_contract.functions.balanceOf(weiroll_vm.address).call()
