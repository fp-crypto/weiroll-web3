...
def whale_account(mainnet_fork_web3, setup_accounts):
    ...
    for raw_address in whale_addresses:
        ...
        try:
            ...
            dai_contract = mainnet_fork_web3.eth.contract(
                address=dai_address, abi=dai_abi
            )
            ...
            if dai_balance >= min_required:
                print(
                    f"Found DAI whale: {whale} with {Web3.from_wei(dai_balance, 'ether')} DAI"
                )

                # Impersonate the whale account
                mainnet_fork_web3.provider.make_request(
                    "anvil_impersonateAccount", [whale]
                )
                ...
                mainnet_fork_web3.provider.make_request(
                    "anvil_stopImpersonatingAccount", [whale]
                )
...
def impersonate_account(
    ...
    if fund_amount is not None and fund_amount > 0:
        tx_hash = web3.eth.send_transaction(
            {"from": dev_account, "to": impersonated, "value": fund_amount}
        )
