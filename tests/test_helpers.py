...
def test_insert(web3, tuple_helper, alice):
    ...
    result = tuple_helper.functions.insertElement(b2 + b1, 0, b4, False).call(
        {"from": alice}
    )
    assert HexBytes(result) == HexBytes(b4 + b2 + b1)

    # Test inserting in the middle
    result = tuple_helper.functions.insertElement(b2 + b1, 1, b4, False).call(
        {"from": alice}
    )
    assert HexBytes(result) == HexBytes(b2 + b4 + b1)

    # Test inserting at the end
    result = tuple_helper.functions.insertElement(b2 + b1, 2, b4, False).call(
        {"from": alice}
    )
    ...
    with pytest.raises(Exception):
        tuple_helper.functions.insertElement(b2 + b1, 3, b4, False).call(
            {"from": alice}
        )
    ...
    for i in range(101):
        result = tuple_helper.functions.insertElement(rands, i, b4, False).call(
            {"from": alice}
        )
...
def test_replace(web3, tuple_helper, alice):
    ...
    result = tuple_helper.functions.replaceElement(b2 + b1, 0, b4, False).call(
        {"from": alice}
    )
    assert HexBytes(result) == HexBytes(b4 + b1)

    # Test replacing the second element
    result = tuple_helper.functions.replaceElement(b2 + b1, 1, b4, False).call(
        {"from": alice}
    )
    ...
    with pytest.raises(Exception):
        tuple_helper.functions.replaceElement(b2 + b1, 2, b4, False).call(
            {"from": alice}
        )
    ...
    for i in range(100):
        result = tuple_helper.functions.replaceElement(rands, i, b4, False).call(
            {"from": alice}
        )
